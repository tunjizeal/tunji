...
class AssetUsageAPITestCase(BaseAssetTestCase):
    ...
    def test_check_api_response(self):
        ...
        assert response.data[0]['asset__name'] == ''
        assert response.data[0]['nlp_usage_all_time']['google_asr_seconds'] == 4728
        assert response.data[0]['nlp_usage_all_time']['google_mt_characters'] == 6726
        assert response.data[0]['storage_bytes'] == 21514156
        assert response.data[0]['submission_count_current_month'] == 2
        assert response.data[0]['submission_count_all_time'] == 2
